...
def save_scan_result(temp_id, scan_data):
    ...
    try:
        ...
        with open(file_path, "w", encoding="utf-8") as file:
            json.dump(existing_data, file, separators=(',', ':'))

        # ✅ Print JSON response correctly
        print("✅ Scan results (JSON format):")
        print(json.dumps(existing_data, separators=(',', ':')))  # Correct print format
...
def get_whois_info(domain, entry_id):
    ...
    try:
        ...
        save_scan_result(entry_id, {"whois_info":json.dumps({'status': 'scanned', 'isError': True, 'err': str(e)})})
...
def get_technologies(domain, entry_id):
    ...
    try:
        ...
        save_scan_result(entry_id, {"new_tech_info" : json.dumps({'error': str(e)})}) 
        return {"error": str(e)}
...
def run_nmap_scan(domain, entry_id):
    ...
    try:
        ...
        for port in open_ports:
            ...
            actualResponse[port] = json.loads(res)
...
def check_missing_headers(domain, entry_id):
    ...
    try:
        ...
        save_scan_result(entry_id, {"missing_headers": json.dumps({'error': str(e)})})
...
def get_http_headers(domain):
    ...
    parsed_url = urlparse(domain)
    print("scanning for http headers")
...
def run_xsstrike(domain,entry_id):
    ...
    #         command = ["dalfox", "url", url, "--output", "json"]
    #         result = subprocess.run(command, capture_output=True, text=True, timeout=180)
    ...
    #                     })
            ...
            save_scan_result(entry_id, {"xss_vuln_data": json.dumps({'error': str(e)})})
...
def enumerate_directories(domain, entry_id):
    ...
    directory_wordlist = load_wordlist("directories2.txt")  # ✅ Load directories list
    ...
    def is_valid_final_page(response):
        suspicious_keywords = ["login", "unauthorized", "access denied", "403 forbidden", "authentication required"]
